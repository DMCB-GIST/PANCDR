...
from ModelTraining.Baseline import train_baseline
...
if __name__ == '__main__':
    ...
    train_data = [X_drug_feat_data,X_drug_adj_data,X_gexpr_data,Y]
    ...
    best_params = eval(df.loc[(df["Model"]=="DeepCDR") & (df["Classification"]=="T"),"Best_params"].values[0])
    model = train_baseline(train_data,test_data)
    results = []
    print("Training......")
    for iter in range(100):
        weight_path = '../checkpoint/TCGA/base_%d_model.pt'%iter
        auc_TCGA = model.train(best_params,weight_path=weight_path)
    ...
    result_df.to_csv('Base_TCGA_100train_results.csv')